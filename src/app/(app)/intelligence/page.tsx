...
const FRAMEWORK_ORDER: RegulatoryFramework[] = [
  "DORA",
  "EU_AI_ACT",
  "SOC2",
  "ISO_27001",
...
export default async function Intelligence() {
  ...
  const [
    { mappings, frameworks, thirdParties },
    summary,
    coverageRaw,
    verifiedRaw,
    triggerStats,
                  ...
                  return (
                    <div key={fw} className="p-lg hover:bg-surface-container-low transition-colors">
                      <div className="flex items-start justify-between mb-md">
                        <div>
                          <span
                            className={`px-2 py-0.5 font-mono-technical text-[10px] border ${meta.color}`}
                          >
                            {fw.replace(/_/g, " ")}
                          </span>
                        ...
                        <span
                          className={`font-mono-technical text-[10px] ${hasData ? "text-primary" : "text-on-surface-variant"}`}
                        >
                      ...
                      <p className="font-mono-technical text-[9px] text-on-surface-variant mb-md">
                        {meta.sector}
                      </p>
                      {hasData ? (
                        <>
            ...
            {[
              {
                ...
                value: `${evidencePct}%`,
                sub: `${summary.evidenceMapped} of ${summary.totalEvidence} items`,
                color: "text-primary",
              },
              {
                label: "GATES CLEARED",
                value: `${summary.totalGates > 0 ? Math.round((summary.approvedGates / summary.totalGates) * 100) : 0}%`,
                sub: `${summary.approvedGates} of ${summary.totalGates} gates`,
                color: "text-primary",
              },
              {
                label: "TRIGGERS MATCHED",
                value: String(triggerMatchCount),
                sub: `${triggerSkipCount} skipped (no rule)`,
                color: "text-tertiary",
              },
              {
                label: "THIRD-PARTY ICIS",
                value: String(thirdParties.length),
                sub: "DORA Art. 28 register",
              ...
              <div key={m.label} className="bg-surface border border-border-muted p-lg">
                ...
                <p
                  className={`font-stat-lg text-stat-lg ${m.color}`}
                  style={{ fontSize: 32, lineHeight: 1 }}
                >
                  {m.value}
                </p>
                <p className="font-mono-technical text-[10px] text-on-surface-variant mt-sm">
                  {m.sub}
                </p>
              </div>
            ))}
              ...
              <div className="divide-y divide-border-muted">
                {policyDocs.length === 0 ? (
                  <div className="p-xl text-center">
                    <p className="font-mono-technical text-[11px] text-on-surface-variant mb-md">
                  ...
                  policyDocs.map((doc) => (
                    ...
                    >
                      ...
                      <span
                        className={`px-2 py-0.5 font-mono-technical text-[10px] border shrink-0 ${doc.indexedAt ? "text-primary border-primary bg-primary/10" : "text-tertiary border-tertiary"}`}
                      >
                ...
                {thirdParties.length === 0 ? (
                  ...
                  thirdParties.map((t) => (
                    ...
                    >
                      ...
                      <span
                        className={`px-2 py-0.5 font-mono-technical text-[10px] uppercase shrink-0 border ${
                ...
                <thead className="bg-surface-container-low border-b border-border-muted">
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className="px-xl py-md font-label-caps text-label-caps text-on-surface-variant text-[10px]"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-border-muted">
                  {mappings.map((m) => (
                    <tr key={m.id} className="hover:bg-surface-container-highest transition-colors">
                      ...
                      <td className="px-xl py-md">
                        <span
                          className={`px-2 py-0.5 font-mono-technical text-[10px] border ${m.verifiedAt ? "bg-primary/10 text-primary border-primary" : "bg-tertiary/20 text-tertiary border-tertiary"}`}
                        >
      ...
      <footer className="h-8 bg-surface-container-low border-t border-border-muted flex items-center justify-between px-xl font-mono-technical text-[10px] text-on-surface-variant shrink-0">
        <div className="flex items-center gap-xl">
          <div className="flex items-center gap-xs">
            ...
            <span>INTELLIGENCE_ENGINE: ACTIVE</span>
          </div>
          <span>
            FRAMEWORKS: {frameworkCount} · MAPPINGS: {totalMappings}
          </span>
        </div>
